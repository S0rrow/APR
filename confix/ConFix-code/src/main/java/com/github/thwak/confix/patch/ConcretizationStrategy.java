...
public class ConcretizationStrategy {
	...
	public void collectMaterials(Node root) {
		System.out.println("Start Collecting Materials");
	...
	protected void collectStaticMembers(ITypeBinding tb, Materials materials) {
		...
		for (IMethodBinding mb : tb.getDeclaredMethods()) {
			if (Modifier.isPublic(mb.getModifiers()) && Modifier.isStatic(mb.getModifiers()) && !mb.isConstructor()) {
				materials.addMethod(mb);
			}
		}
	}
	...
	/**
	 * Check whether all types, variables, and methods in the given change can be
	 * instantiated with collected values from the given target location.
	 * 
	 * @param change a change to be applied.
	 * @param loc    a target location where the change will be applied.
	 * @return {@code true} if the given {@code change} can be instantiated with
	...
	public boolean instCheck(Change change, TargetLocation loc) {
		...
		if (change.type.equals(Change.UPDATE) && loc.kind == Node.K_TYPE) {
		...
		for (String absSignature : reqs.methods.keySet()) {
			...
			if (!materials.methods.containsKey(absSignature)) {
				...
				if (change.type.equals(Change.UPDATE) && loc.node.kind == Node.K_METHOD) {
					count--;
				}
	...
	/**
	 * Adapt a given abstract change {@code c} to a given target location
	 * {@code loc} with collected materials, and returns an {@code ASTNode} which
	 * can be used for change application.
	 *
		...
		if (reqs.variables.size() > 0) {
			...
			if (c.type.equals(Change.UPDATE) && c.node.type == loc.node.type && c.node.kind == loc.node.kind) {
				...
				if (materials.variables.containsKey(type))
					for (Variable v : materials.variables.get(type))
						if (!oldName.equals(v.name))
							locVariables.add(v);
				Set<Variable> localVars = materials.getLocalVariables(loc.methodKey, type, loc.blockIndex,
						loc.node.startLine);
				...
				for (VariableType type : reqs.variables.keySet()) {
					...
					locVariables.addAll(
							materials.getLocalVariables(loc.methodKey, type, loc.blockIndex, loc.node.startLine));
	...
	protected boolean assignTypes(Map<VariableType, VariableType> typeMap, Requirements reqs, Materials materials,
			boolean isGeneric, VariableType oldType) {
		...
		for (VariableType type : reqTypes) {
			if (!typeMap.containsKey(type)) {
				...
				if (reqVarCount > 0 || reqFieldCount > 0 || reqDeclaredFieldCount > 0) {
					List<VariableType> candidates = new ArrayList<>();
					for (VariableType locType : remainingTypes) {
						int locVarCount = materials.variables.containsKey(locType)
								? materials.variables.get(locType).size()
								: 0;
						int locFieldCount = materials.fields.containsKey(locType) ? materials.fields.get(locType).size()
								: 0;
						...
						if (locVarCount >= reqVarCount && locFieldCount >= reqFieldCount
	...
	protected boolean checkCompatible(Method cMethod, Method locMethod, Requirements reqs, Materials materials,
		...
		for (Entry<VariableType, VariableType> entry : mapping.entrySet()) {
			if (typeMap.containsKey(entry.getKey())) {
				...
				int reqVarCount = reqs.variables.containsKey(entry.getKey()) ? reqs.variables.get(entry.getKey()).size()
						: 0;
				int reqFieldCount = reqs.fields.containsKey(entry.getKey()) ? reqs.fields.get(entry.getKey()).size()
						: 0;
				int locVarCount = materials.variables.containsKey(entry.getValue())
						? materials.variables.get(entry.getValue()).size()
						: 0;
				int locFieldCount = materials.fields.containsKey(entry.getValue())
						? materials.fields.get(entry.getValue()).size()
						: 0;
				if (reqVarCount > locVarCount || reqFieldCount > locFieldCount)
					return false;
			}
		}
		return true;
	...
	public void collectGlobalMaterials(Parser parser, String className, Node root, CompilationUnit cu) {
		...
		for (ImportDeclaration decl : importDecls) {
			...
			if (tb != null) {
				VariableType type = MVTManager.generateType(tb);
				if (type.isGeneric) {
					// In case of generic types, it's possible that types in JSL are included in
					// requirements.
